		...
		case "$op" in
			...
			find)
				...
				count_op=$((count_op+1))
		   		;;
			buildinfo)
				tshark -r $1 -w $2/buildinfo-$count_build.cap -Y "mongo.request_id==$seqnum"
				tshark -r $1 -w $2/Operation/operation-$count_op.cap -Y "mongo.request_id==$seqnum"
				count_build=$((count_build+1))
				count_op=$((count_op+1))
		   		;;
			getlasterror)
				tshark -r $1 -w $2/getlasterror-$count_getlast.cap -Y "mongo.request_id==$seqnum"
				tshark -r $1 -w $2/Operation/operation-$count_op.cap -Y "mongo.request_id==$seqnum"
				count_getlast=$((count_getlast+1))
				count_op=$((count_op+1))
		   		;;
			ismaster)
				tshark -r $1 -w $2/ismaster-$count_ismaster.cap -Y "mongo.request_id==$seqnum"
				tshark -r $1 -w $2/Operation/operation-$count_op.cap -Y "mongo.request_id==$seqnum"
				count_ismaster=$((count_ismaster+1))
				count_op=$((count_op+1))
		   		;;
			*)
				echo "others="$op
